...
create table if not exists public.badges (
  ...
  max_win_pct numeric(5,2),
  min_attendance integer,
  max_attendance integer,
  min_point_diff integer,
  max_point_diff integer,
  min_paid_rate numeric(5,2),
  max_paid_rate numeric(5,2),
  match_window integer,
  attendance_window integer,
  payment_within_hours numeric(6,2),
  fallback_type text,
  enabled boolean not null default true,
  ...
  constraint badges_max_win_pct_check check (max_win_pct is null or (max_win_pct >= 0 and max_win_pct <= 100)),
  constraint badges_max_attendance_check check (max_attendance is null or (max_attendance >= 0 and max_attendance <= 100000)),
  constraint badges_max_point_diff_check check (max_point_diff is null or (max_point_diff >= -100000 and max_point_diff <= 100000)),
  constraint badges_min_max_win_pct_check check (min_win_pct is null or max_win_pct is null or max_win_pct >= min_win_pct),
  constraint badges_min_max_attendance_check check (min_attendance is null or max_attendance is null or max_attendance >= min_attendance),
  constraint badges_min_max_point_diff_check check (min_point_diff is null or max_point_diff is null or max_point_diff >= min_point_diff),
  constraint badges_min_max_paid_rate_check check (min_paid_rate is null or max_paid_rate is null or max_paid_rate >= min_paid_rate),
  ...
  constraint badges_max_paid_rate_check check (max_paid_rate is null or (max_paid_rate >= 0 and max_paid_rate <= 100)),
